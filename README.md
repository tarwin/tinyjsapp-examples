...
In plain zero-dependency JavaScript — and not just text: **images, copied files, and colors too**. Built on 0.11.0's **native clipboard** (`app.clipboard.read()/write()/changeCount()` — no `pbpaste`, no polling spawns): one capture call classifies files → image → color → text and keeps rich text's html flavour; a single JXA probe per change adds what core doesn't expose yet (Concealed-flag so password managers stay private, "from app" attribution, and the browser source URL — **⌘O** reopens the page). Everything upserts into SQLite (`tjs:sqlite`); images land on disk with `sips` thumbnails, and image/file clips **drag out of the palette as real files** (`win.startDrag`). **⌘⇧V** anywhere summons a frameless vibrancy palette: ⏎ puts a clip back *as whatever it was* and `hide()` lands you straight back in the app you came from — **⌥⏎ then pastes it there** (`app.paste()`, native ⌘V, permission prompted via `app.permissions`), ⇧⏎ strips rich text, ⌘P pins (pinned clips survive pruning and Clear History).
